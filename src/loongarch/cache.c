...
{
	*l1i = (struct cpuinfo_cache) { 0 };
	*l1d = (struct cpuinfo_cache) { 0 };
	*l2 = (struct cpuinfo_cache) { 0 };
	*l3 = (struct cpuinfo_cache) { 0 };
		...
		*/
		...
		case cpuinfo_uarch_LA664:
			...
			*l3 = (struct cpuinfo_cache) {
				.size = cluster_cores <= 4 ? 16 * 1024 * 1024 : 32 * 1024 * 1024,
				.associativity = 16,
				.line_size = 64
			};
			break;
		default:
			cpuinfo_log_warning("loongarch uarch not recognized; cache parameters are unavailable");
			break;
	}
	if (l1i->size != 0) {
...
uint32_t cpuinfo_loongarch_compute_max_cache_size(
	...
	switch (processor->core->uarch) {
		...
		case cpuinfo_uarch_LA464:
		case cpuinfo_uarch_LA664:
			if (processor->cache.l3 != NULL && processor->cache.l3->size != 0) {
